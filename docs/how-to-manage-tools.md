...
Tool permissions only apply when the active model is **Claude**. With Codex, Gemini, Gajae-Code, or OpenCode selected, `/allowed` is rejected with `Tool permissions are not supported in this mode.` The other tool commands (`/availabletools`, `/allowedtools`) still work for inspection.
