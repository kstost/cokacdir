  ...
  return (
    <div>
      ...
      <SubSection title={String(t('Loop — Self-Verification Loop', '루프 — 자가 검증 반복'))}>
        ...
        <ul className="list-disc list-inside space-y-2 text-zinc-400 my-4 ml-2">
          <li>{t(
            <><strong className="text-zinc-300">Claude, Codex, or OpenCode model.</strong> Each provider uses its own isolation mechanism for verification: Claude uses the native <IC>--fork-session</IC>, Codex runs an independent <IC>codex exec --ephemeral</IC> with a transcript synthesized from the full-fidelity archive (original rollout file stays byte-identical), and OpenCode uses the native <IC>opencode run --session &lt;id&gt; --fork --agent plan</IC>. Gemini and Gajae-Code are still rejected.</>,
            <><strong className="text-zinc-300">Claude, Codex, OpenCode 모델 지원.</strong> 각 프로바이더는 검증에 자체 격리 방식을 사용합니다: Claude는 네이티브 <IC>--fork-session</IC>, Codex는 full-fidelity 아카이브에서 트랜스크립트를 합성해 독립 <IC>codex exec --ephemeral</IC>을 실행(원본 rollout 파일 바이트 단위로 불변), OpenCode는 네이티브 <IC>opencode run --session &lt;id&gt; --fork --agent plan</IC>을 사용합니다. Gemini와 Gajae-Code는 거부됩니다.</>
          )}</li>
          <li>{t(
      ...
      <SubSection title={String(t('Codex image_gen — Auto-Delivered Images', 'Codex image_gen — 이미지 자동 전달'))}>
        ...
        <InfoBox type="info">
          {t(
            'This auto-delivery is Codex-specific — Claude Code, Gemini, Gajae-Code, and OpenCode do not write to that directory and are unaffected.',
            '이 자동 전달은 Codex 전용입니다 — Claude Code, Gemini, Gajae-Code, OpenCode는 해당 디렉터리에 쓰지 않으며 영향을 받지 않습니다.'
          )}
        </InfoBox>
      </SubSection>
    </div>
